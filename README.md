...
This project implements a **local Power BI Embedded (PBIE) development server** that serves iframe-embedded analytics powered by a **Fabric semantic model**, with a **Fabric Data Agent** as the semantic model access layer for both the report and the chat assistant.
...
Browser
  ...
  │
  └── AI Chat Panel ──► Node.js/Express backend ──► Fabric Data Agent
        (conversationId,        (context service:              │
         question)               field_map.json)                ▼
                                                     Fabric semantic model query
                                                                  │
                                                                  ▼
                                            Power BI XMLA endpoint (Invoke-ASCmd shim)
                                                                  │
                                                                  ▼
...
| AI reasoning layer | Direct Fabric Data Agent query (`server/services/fabricAgent.js`) | Maps each question to a DAX query shape and returns synthesized natural-language answers, without a separate LLM orchestration layer |
...
4. Open `http://localhost:3000` — the iframe renders the live report and the Analytics Assistant chat panel is wired directly to the Fabric Data Agent query layer
...
| Sprint 2 | Context Capture to JSON | **Complete** — context flows end-to-end into the chat backend's semantic model query prompts |
...
| Sprint 4 | Context Injection Chat Layer | **Complete** — end-to-end validated: chat → direct Fabric Data Agent query → real data → natural-language answer |
| Sprint 5 | Semantic Query Layer Refinement | **Complete** — 10 DAX query shapes, `field_map.json` rewrite, context service |
...
**All 9 sprints complete.** The project is demo-ready end-to-end: iframe → context capture → direct Fabric Data Agent query → live semantic model query → natural-language answer, the headline KPI cards show validated, correct year-over-year figures, and identity/RLS enforcement no longer trusts any client-supplied identifier.
