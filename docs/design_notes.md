...
| Secret hygiene | `CLIENT_SECRET` for `VISA-PBIE-EmbedService` | ✅ **Rotated 2026-07-22** after exposure in a chat attachment — post-rotation smoke test confirmed SP auth, embed token generation, RLS, and XMLA connectivity all unaffected. The Fabric-side fixed-identity cloud connection (`VISA-PBIE-FixedIdentity-DirectLake`) held the pre-rotation secret and caused a `Dataset_Import_FailedToImportDataset`/`AADSTS7000215` error on the next Fabric Git sync — fixed via `PATCH /v1/connections/{id}` (Fabric REST API), no portal click required |
