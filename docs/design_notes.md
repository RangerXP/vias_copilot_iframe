...
| Fail-closed hardening: reject/error when no entitlement resolves for a user, instead of silently falling back to the SP's full/unfiltered view | ✅ **Resolved 2026-07-22** — `server/routes/embedToken.js` now rejects explicitly (`401` if no `user` supplied, `403` if the user doesn't resolve to a known entitlement/role) before ever calling `GenerateToken`, instead of relying solely on Power BI's own "requires effective identity" `400` |
...
## 19. Post-Foundry-removal regressions found & fixed (2026-07-23/24)

> Two issues surfaced while validating a fresh multi-tenant install (`Setup-Tenant.ps1` test-tenant workspace `b7055e05-...`). Neither is a security-model change — the XMLA + `Role_Entitlement` + fixed-identity Direct Lake design from Sections 15-17 is unchanged and still in effect (confirmed via `git show` of the Foundry-removal commit — it touched exactly one line, a JSDoc comment in `fabricAgent.js`).

**19a. Chat answers regressed to raw JSON.** Before the Foundry Agent was removed, `queryFabricAgent()`'s `JSON.stringify(...)` output was fed to the Foundry LLM as tool output, and the LLM (per the "synthesis fallback" fix from an earlier sprint) turned it into natural-language prose before it reached the user. Once `server/routes/chat.js` was changed to call `queryFabricAgent()` directly, that raw JSON string started going straight to the chat UI unchanged — a real regression, just not a security one. **Fix:** added a deterministic `formatAnswer()` helper in `server/services/fabricAgent.js` that renders the normalized rows as natural-language text (no LLM involved, consistent with the decision to not reintroduce an orchestration layer) — a single narrative sentence for `{Metric, Value}` summary rows, a bulleted breakdown for category+measure rows. Both the XMLA and `executeQueries` code paths now return `formatAnswer(...)` instead of `JSON.stringify(...)`.

**19b. Fixed-identity Direct Lake connection was never created for new tenant workspaces.** `Setup-Tenant.ps1` previously only printed a manual instruction to bind a fixed-identity connection in the Fabric portal — it never created the connection object itself, even though connection *creation* (unlike the dataset-bind step) is fully API-doable. **Fix:** the ServicePrincipal phase now creates an `AzureDataLakeStorage` cloud connection (SP credentials, SSO disabled) pointed at the new workspace's own lakehouse, and grants the signed-in user `Owner` on the connection object — otherwise the connection exists but is invisible in the portal's "Gateway and cloud connections" dropdown (connections are only visible to principals with an explicit role assignment on them, and by default only the creating SP has one). The one remaining manual, non-automatable step is selecting that connection in the dataset's datasource dropdown and clicking Apply (no supported public API for that bind).

Both fixes committed to `branch` — see git log around 2026-07-23/24 for exact diffs (`server/services/fabricAgent.js`, `scripts/Setup-Tenant.ps1`).

## 20. Lightweight LLM Orchestrator (reinstated 2026-07-24/25)

> Answers the question raised after the RLS analysis: "is there a role-entitlement fix that mirrors the XMLA/CUSTOMDATA() solution for the native Fabric Data Agent, or do we need an LLM orchestrator back in front of it?" Confirmed via Microsoft Learn docs that the Fabric Data Agent's conversational API (`USE_DATA_AGENT`, disabled since Section 19's sibling gap-finding) always executes under the semantic model's fixed-identity connection — there is no per-caller identity/entitlement parameter, and a service principal cannot hold RLS role membership the way a real Entra user can. That gap cannot be closed without a genuine delegated user token (EffectiveIdentity/OBO), which this app's synthetic demo-customer sessions don't have.

**Decision:** reinstate an LLM orchestrator, but keep it lightweight — no separate Azure AI Foundry Agents project/thread/assistant resources (that was the pre-removal architecture). Instead, `server/services/llmOrchestrator.js` calls Azure OpenAI chat completions directly via `fetch()` (Entra ID auth via `DefaultAzureCredential`, scope `https://cognitiveservices.azure.com/.default` — no API key), with function-calling exposing exactly ONE tool, `get_commercial_spend_data(shape)`, whose `shape` enum is the existing `DAX_SHAPES` registry (`summary, trend, segment, merchant, country, product, mcc, approval, fraud, industry`). The tool is executed by this project's own code via `executeDax()`/`runXmlaQuery()` — the same RLS-enforcing path every other question class already uses. `effectiveUserName`/`rlsMode` are always bound from the authenticated session server-side; the model only ever chooses *which* data shape to fetch, never *who* it's fetched for, so this cannot become an RLS-bypass surface the way the native Data Agent's conversational API is.

Wired into `queryFabricAgent()` behind `USE_LLM_ORCHESTRATOR=true`, ahead of the (still-disabled) `USE_DATA_AGENT` branch, with the same try/catch-and-fallback-to-deterministic-DAX convention used everywhere else in that function. No new npm dependency (raw `fetch()`, consistent with the rest of the codebase). Reuses an already-provisioned Cognitive Services `AIServices` account (`FoundryWest3`, `gpt-4o` deployment) rather than provisioning new Azure infrastructure — the project's SP was granted "Cognitive Services OpenAI User" scoped to that account.

Validated live, both demo identities: multi-tool-call questions (e.g. "total interchange revenue AND top merchant category") correctly issue two tool calls in one turn, and the RLS-scoped figures returned exactly match the deterministic Key Metrics table for the same session (North America vs. Europe give different, correctly-scoped numbers).
